...
class charLSTMmodel:
    ...
    def generate(self, words, i=150):
        for _ in range(i):
            x = [
                self.token_idx[token] if token in self.token_idx else 1
                for token in self._tokenise(words)
            ]
            x = preprocessing.sequence.pad_sequences(
                [x], maxlen=self.x_dim, padding="pre"
            )
            y_hat = self.model.predict_classes(x, verbose=0)[0]  # maximise
            words += self.idx_token[y_hat]
        return words.replace("_", " ")

    def _chunk(self, text, chunk_size=10000):
        return "".join(
            [
                c + "<S>" if not i % chunk_size else c
                for i, c in enumerate(text, start=1)
            ]
        ).split("<S>")

    def _tokenise(self, text):
        return list(" ".join(text).replace(" ", "_"))
    ...
    def _load(self, text):
        ...
        token_ids = [
            [self.token_idx[token] for token in self._tokenise(sentence)]
            for sentence in self._chunk(text)
        ]
        inouts = [
            tokens[: i + 1] for tokens in token_ids for i in range(1, len(tokens))
        ]
        self.x_dim = max([len(x) for x in inouts]) - 1
        self.y_dim = len(self.idx_token)
        inouts = np.array(
            preprocessing.sequence.pad_sequences(
                inouts, maxlen=self.x_dim + 1, padding="pre"
            )
        )
